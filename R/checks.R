...
check_isInBounds <- function(param){
  ...
  for(i in 1:length(names_phy)){

    # check min bound
    mat_isInBounds['min',names_phy[i]] <-
      (param[names_phy[i]] >= param[[paste0(names_phy[i],'_min')]])

    # check max bound
    mat_isInBounds['max',names_phy[i]] <-
      (param[names_phy[i]] <= param[[paste0(names_phy[i],'_max')]])
  }
